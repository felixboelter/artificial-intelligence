...
class ActionLayer(BaseActionLayer):
    ...
    def _interference(self, actionA, actionB):
        ...
        """
        # TODO: implement this function
        for preconditionA in actionA.effects:
            if ~preconditionA in actionB.preconditions:
                return True
        for preconditionB in actionB.effects:
            if ~preconditionB in actionA.preconditions:
                return True
    ...
    def _competing_needs(self, actionA, actionB):
        ...
        """
        # TODO: implement this function
        return any(self.parent_layer.is_mutex(preconA, preconB) for preconA in actionA.preconditions for preconB in actionB.preconditions)
...
class PlanningGraph:
        ...
        while not self._is_leveled:
            ...
            for goal in self.goal:
                if goal not in self.literal_layers[-1]:
                    allGoalsMet = False
                if goal in self.literal_layers[-1] and goal not in goals:
                    sol += level
                    goals.append(goal)
            if allGoalsMet:
                return sol
    ...
    def h_setlevel(self):
        ...
        while not self._is_leveled:
            ...
            for gA in self.goal:
                for gB in self.goal:
                    if self.literal_layers[-1].is_mutex(gA,gB):
                        goalsAreMutex=True
